...
Write a function `calculate_euclidean_distance_between_points` that takes an argument `points`, representing a list of points in a [two-dimensional Cartesian coordinate system](https://en.wikipedia.org/wiki/Cartesian_coordinate_system), and that returns the sum of the [Euclidean distance](https://en.wikipedia.org/wiki/Euclidean_distance) between each consecutive point in this list.
...
You **MUST** use the function `char_to_int` that you have written in the previous waypoint.
...
Write a function `roman_numeral_to_int` that take an argument `roman_numeral` (a string) and returns the integer value corresponding to this Roman numeral.

The function should raise the exception `TypeError` if the argument `roman_numeral` is not a string.

The function should raise the exception `ValueError` if the argument `roman_numeral` is an empty string or some characters of the string `roman_numeral` don't represent Roman numeral symbols.
...
The accidentals are written after the note name. For example, F♯ represents F-sharp, B♭ is B-flat.

_Note: For practical reasons, we will use the character `#` and `b` to represent the accidentals **sharp** (♯) and **flat** (♭). Accidentals would be easier to type on a computer keyboard. For instance `E♭5` is noted `Eb5`, and `F♯6` is noted `F#6`._
...
_You might think that you are playing notes one after one other, but your code may be playing a new note while the previous note is still playing. If you look closely to the `.ogg` files, the total duration of a note is several seconds._
...
We have said that accidental **sharps** (♯) raises a note by a semitone, while accidental **flats** (♭) lowers a note by the same amount. This means that a note with an accidental **sharp** is equivalent to the next note with an accidental **flat**. For instance, `F♯4` is equivalent to `G♭4`.
